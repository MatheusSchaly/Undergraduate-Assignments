...
int T = 200; // Asserts if a process is old or not
...
{
	int idx;      /* Loop index.  */
	int remove = -1; /* Page to remove */
	int replace_found = 0;	/* Found a page to replace */
	int pointer = curr_proc->pointer; /* Processes pointer for circular list */
  int age;  /* Calculates page age */
	int oldest = -1; /* Oldest page */
	int oldest_idx = -1; /* Oldest index page */
	struct pte *pg; /* Working page table entry. */
	
	/* Search for a free frame. */
	for (idx = pointer; idx != pointer-1; idx++)
	{
		...
		/* Get working page table entry. */
		pg = getpte(curr_proc, frames[idx].addr);
		
		/* Found it. */
		if (frames[idx].count == 0)
			goto found;			
		
		/* Local page replacement policy. */
		if (frames[idx].owner == curr_proc->pid)
		{
			/* Skip shared pages. */
			if (frames[idx].count > 1)
				continue;
				
			/* If R == 1 then set R = 0, copy CVT (time) to TLU, and continue */
			if (pg->accessed == 1) {
				pg->accessed = 0;
				frames[idx].tlu = curr_proc->time;
			/* If R == 0 then calculate age = CVT (time) - TLU */
			} else {
				age = curr_proc->time - frames[idx].tlu;
				/* If age <= T stores biggest age and continue */
				if (age <= T) {
					if (age > oldest) {
						oldest = age;
						oldest_idx = idx;
					}
					/* If age > T and M == 0 then replace page */
				} else if (age > T && pg->dirty == 0) {
					replace_found = 1;
					remove = idx;
					pointer = idx;
					continue;
					/* If age > T and M == 1 then write on disk and continue */
				} else if (age > T && pg->dirty == 1) {
					; // Do nothing for now
				}
			}
		}
	...
	/* Set processes pointer location */
	if (idx == NR_FRAMES-1) {
		curr_proc->pointer = 0;
	} else {
		curr_proc->pointer = idx+1;
	}
	
	/* After one complete round and didn't find page to remove, remove oldest (not WSClock)*/
	if (replace_found == 0) {
		remove = oldest_idx;
	}
	
	/* Swap page out. */
	if (swap_out(curr_proc, frames[idx = remove].addr)) {
		return (-1);
	}
