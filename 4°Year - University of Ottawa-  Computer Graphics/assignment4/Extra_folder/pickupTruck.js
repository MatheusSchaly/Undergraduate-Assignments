...
var front_human_eye = new THREE.Vector3(0, 0, 0);
var object;
...
function main() {
	...
	cameraOrtho = new THREE.OrthographicCamera(frustum_var * aspect / - 2, frustum_var * aspect / 2, frustum_var * aspect / 2 * 0.2, frustum_var * aspect / 2 * -0.2, 50, far_view_ortho);
	...
	cameraPerspe.position.z = -0.5;
	
	scene.add(cameraPerspeHelper);
	...
	var wheelMaterial = new THREE.MeshPhongMaterial({color: 'white'});
	...
	var controls = new function () {
		...
		this.change_driver_view = function () {
			if (back_driver_view) {
				cameraPerspe.position.z += 16;
				back_driver_view = false;
			} else {
				cameraPerspe.position.z -= 16;
				back_driver_view = true;
			}
		};
	...
	gui.add(controls, 'change_driver_view');
	...
	function render() {
		...
		cameraPerspe.position.z += zSpeed

		if (back_driver_view) {
			back_human_eye.z = pickupTruck.position.z - 40;
		} else {
			back_human_eye.z = pickupTruck.position.z + 40;
		}
		
		back_human_eye.y = pickupTruck.position.y + 19;
		cameraPerspe.updateProjectionMatrix();
		cameraPerspeHelper.update();
		cameraPerspe.lookAt(back_human_eye);
