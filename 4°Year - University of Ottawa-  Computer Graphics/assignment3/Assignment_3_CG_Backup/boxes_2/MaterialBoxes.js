...
function start(gl) {
  ...
  glMatrix.vec4.set(lights.diffuse, 0.5, 0.5, 0.5, 1);
  glMatrix.vec4.set(lights.position, 15, 0, 0, 1);

  // Initialize lights
  var lghtCtrl = [];	
	var lghtCtrl = (new function() {
		this.spot_light = (lights.spot_light > 0);
		this.spot_exponent = lights.spot_exponent;
		this.spot_cutoff = lights.spot_cutoff;
		this.constant_attenuation = lights.constant_attenuation;
		this.linear_attenuation = lights.linear_attenuation;
		this.quadratic_attenuation = lights.quadratic_attenuation;
		this.diffuse = [ Math.trunc(255*lights.diffuse[0]),
				 Math.trunc(255*lights.diffuse[1]),
				 Math.trunc(255*lights.diffuse[2])];
		this.specular = [ Math.trunc(255*lights.specular[0]),
				  Math.trunc(255*lights.specular[1]),
				  Math.trunc(255*lights.specular[2])];
		this.ambient = [ Math.trunc(255*lights.ambient[0]),
				 Math.trunc(255*lights.ambient[1]),
				 Math.trunc(255*lights.ambient[2])];
		this.directional = (lights.position[3] < 1e-6);
		this.light = lights;
		
		this.setLight = function() {
		  this.light.setLight(gl, gl.program, 0);
		  this.light.setPosition(gl, gl.program, 0);
		}
  ...
  document.onkeydown = function(ev){
      keydown(ev, gl, n, currentAngle, axisAngle, u_rot_matrix, lights[1], 1 )};
...
function setLightMenu( fldr, controls ) {
  ...
  fldr.add(controls, 'directional').onChange(controls.setDirectional);
...
function initVertexBuffers(gl) {
  ...
  let minV = glMatrix.vec3.fromValues(-10.0,-10.0,-30.0);
...
function keydown(ev, gl, n, currentAngle, axisAngle, u_rot_matrix, lght, index) {
  ...
  glMatrix.vec4.set(lght.position,15.0*Math.cos(ANGLE),15.0*Math.sin(ANGLE),0.0,1.0);  
