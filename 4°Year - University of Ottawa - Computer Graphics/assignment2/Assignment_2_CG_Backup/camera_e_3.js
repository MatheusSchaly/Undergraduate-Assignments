...
function init() {
	...
	cameraOrtho_1 = new THREE.OrthographicCamera( -d_width/2.0 * 3 * aspectRatio, d_width/2.0 * 3 * aspectRatio, d_height/2.0, -d_height/2.0, d_near * 3, d_far * 3);
	cameraOrthoHelper_1 = new THREE.CameraHelper( cameraOrtho_1 );
	scene.add( cameraOrthoHelper_1 );

	cameraOrtho_2 = new THREE.OrthographicCamera( 1.5 * frustumSize * aspectRatio / - 2, 1.5 * frustumSize * aspectRatio / 2, frustumSize / 2, frustumSize / - 2, d_near, d_far + 5);
	cameraOrthoHelper_2 = new THREE.CameraHelper( cameraOrtho_2 );
	scene.add( cameraOrthoHelper_2 );
	...
	function render() {
		...
		renderer.render(scene, cameraOrtho_2);
	}
	...
	function updateLookAt() {
		...
		camera.lookAt(at);
		camera.updateMatrix();
		cameraOrtho_1.updateProjectionMatrix();
		cameraOrtho_2.updateProjectionMatrix();
	}
