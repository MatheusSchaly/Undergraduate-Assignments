...
void *produtor_func(void *arg) {
    ...
    for (int i = 0; i <= max; ++i) {
        int produto;
        if (i == max)
            produto = -1;          //envia produto sinalizando FIM
        else
...
int main(int argc, char *argv[]) {
	...
	pthread_t t_consumidor, t_produtor;

	pthread_create(&t_produtor, NULL, produtor_func, (void *) &n_itens);
	pthread_create(&t_consumidor, NULL, consumidor_func, NULL);
