...
# Next, edit the file to add a few useful entries to each of the three predicates, 
# and run more queries to see that you have not spoiled anything.

# 1 - book(X, Y, 'Springer', Z, W).
# 2 - book(A, B, C, D, E), D > 1990.
# 3 - reader(X,'James Brown'), checkedOut(X, Y), book(A, B, C, D, Y).
