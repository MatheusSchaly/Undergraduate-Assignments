...
public class StableMatching {
	...
	public Set<Match> Gale_Shapley(ArrayList<Employer> employers, ArrayList<Student> students) {
		...
		while (!all_matched()) { // Checks if there is a employer that is not matched yet
			Employer employer = null;
			for (Employer temp_employer : employers) { 
				if (temp_employer.get_match() == null) {
					employer = temp_employer; // Gets the unmatched employer
					break;
				}
			}
			Student best_student = employer.get_preferences().get(0); // Get the current best student for that employer
			if (best_student.get_match() == null) { // If the best student doesn't have a match
				Match new_match = new Match(); // Creates a new match
				new_match.set_employer(employer); // Adds employer and
				new_match.set_student(best_student); // Adds student to the match
				employer.match(best_student); // Matches the employer with the student
				best_student.match(employer); // Matches the student with the employer
				M.add(new_match); // Adds the new match to the final match set
			} else if (best_student.get_match_rank(employer) < best_student.get_current_match_rank()) { // If the new employer has better rank than older
				for (Match match : M) { // Searches for the old student match
					if (match.get_student().equals(best_student)) {
						match.get_employer().match(null); // Sets the previous employer match to null
						match.set_employer(employer); // Sets a new employer to the student match
						employer.match(best_student); // Matches the new employer to the student
						break;
					}
				}
			}
			employer.get_preferences().remove(best_student); // Removes student from employer so that the student is no analyzed again by the same employer
		}
		return M; // Return final set which contains the final answer
	}
